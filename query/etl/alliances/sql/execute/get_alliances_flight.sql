WITH aux AS (SELECT DISTINCT factsegment.rec_loc                                       AS at_cd_rec_loc,
                             ...
                             CASE
                                 WHEN factsegment.isLevel = 1
                                     THEN 'LV'
                                 WHEN factsegment.placa = '075'
                                     THEN 'IB'
                                 WHEN factsegment.placa = '125'
                                     THEN 'BA'
                                 WHEN factsegment.placa = '157'
                                     THEN 'QR'
                                 WHEN factsegment.placa IN ('045', '957')
                                     THEN 'LA'
                                 WHEN factsegment.placa = '001'
                                     THEN 'AA'
                                 ...
                                 WHEN factsegment.placa = '607'
                                     THEN 'EY'
                                 ELSE flight.mktCarrierCode
                                 END                                                   AS at_cd_carrier,
                             CASE
                                 WHEN next_segment.placa = '030' AND next_segment.isLevel IS NULL AND next_segment.is_cx = 1
                                     THEN 'VY'
                                 WHEN next_segment.placa = '030' AND next_segment.isLevel = 1 AND next_segment.is_cx = 1
                                     THEN 'LV'
                                 WHEN next_segment.placa = '075' AND next_segment.is_cx = 1
                                     THEN 'IB'
                                 WHEN next_segment.placa = '125' AND next_segment.is_cx = 1
                                     THEN 'BA'
                                 WHEN next_segment.placa = '157' AND next_segment.is_cx = 1
                                     THEN 'QR'
                                 WHEN next_segment.placa IN ('045', '957') AND next_segment.is_cx = 1
                                     THEN 'LA'
                                 WHEN next_segment.placa = '001' AND next_segment.is_cx = 1
                                     THEN 'AA'
...
ON flight.airport_orig = r.AT_CD_ORIG and flight.airport_dest = r.AT_CD_DEST
    ...
    JOIN Vueling_data_master.dbo.DIM_FEE_TYPE dft
    WITH (NOLOCK) ON dft.ID_FEE_TYPE = fee.ID_FEE_TYPE
...
WHERE 1 = 1
  ...
  AND (factsegment.AT_CD_MKT_CARRIER_CODE = ''
   OR factsegment.AT_CD_MKT_CARRIER_CODE IS NULL)
    )
SELECT *,
       CASE

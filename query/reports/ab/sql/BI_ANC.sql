...
(
  ...
  FROM
    dbo.FactFlight RIGHT OUTER JOIN DMV_Vueling_ventas.dbo.V_FACT_FEE_ACTIVE ON (dbo.FactFlight.flight_sk=DMV_Vueling_ventas.dbo.V_FACT_FEE_ACTIVE.ID_FLIGHT)
    INNER JOIN dbo.FactPNR ON (dbo.FactPNR.rec_loc=DMV_Vueling_ventas.dbo.V_FACT_FEE_ACTIVE.AT_CD_PNR)
    INNER JOIN DMV.dbo.TA_rec_loc_otros1 ON (DMV.dbo.TA_rec_loc_otros1.rec_loc=dbo.FactPNR.rec_loc)
    INNER JOIN dbo.DIM_FEE_TYPE ON (DMV_Vueling_ventas.dbo.V_FACT_FEE_ACTIVE.ID_FEE_TYPE=dbo.DIM_FEE_TYPE.ID_FEE_TYPE)
  WHERE
  ( dbo.DIM_FEE_TYPE.AT_DS_REVENUE_CONCEPT_1  not in ('TICKETS','OTHER')  )
    AND  
    (
    ...
    AND
    DMV_Vueling_ventas.dbo.V_FACT_FEE_ACTIVE.MT_FULL_UNIT_PRICE_AMOUNT  <>  0
    )
  GROUP BY
    dbo.DIM_FEE_TYPE.AT_DS_REVENUE_CONCEPT_3_GROUPED, 
    DMV.dbo.TA_rec_loc_otros1.rec_loc, 
...
SELECT q1.rec_loc,
        ...
        q1.AT_DS_REVENUE_CONCEPT_3_GROUPED AS "Ancillary Category",

	...
	from (select
			...
			from (select
					...
					from (select
							...
							p.arc_iata as iata,
							count(*) as sum_pax,
							case when pp.first_name like '%TBA%' then 1 else 0 end as tba
						from dbo.factPnr p
						...
						where
							flight_date between cast(getdate() as date) and DATEADD(dd, 60, cast(getdate() as date))
						...
						group by
							...
							fare_basis_nav,
							p.booking_date,
							case when first_name like '%TBA%' then 1 else 0 end, arc_iata
			) b
			group by
				pnr,
				numvuelo,
...
left join dbo.factflight ff on cast(ff.flight_date as date) = c.fecha and ff.flight_number = c.numvuelo and idstatus = 1
inner join vueling_navitaire.dbo.Booking b WITH(NOLOCK) on b.recordlocator = c.pnr
...
left join (select AT_CD_PNR as pnr, count(distinct f.AT_CD_PAX_NBR) as seat, ID_FLIGHT
            from fees.fact_fee f
                join Vueling_data_master.dbo.DIM_FEE_TYPE t on f.ID_FEE_TYPE=t.ID_FEE_TYPE
            where ot_dt_end_date='29991231'
                and t.AT_CD_SERVICE_TYPE='SEAT'
            group by AT_CD_PNR,ID_FLIGHT) fe on fp.rec_loc=fe.pnr and fe.id_flight=ff.flight_sk
)  TbaSAS
WHERE
    TbaSAS.farebasis = 'GBPTG'
    AND cast(fecha as date) between '{0}' and '{1}'
    AND   TbaSAS.is_tba = 'TBA';

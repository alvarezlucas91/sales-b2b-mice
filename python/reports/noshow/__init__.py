...
def main(start_date: str, end_date: str):
    df = gather_data(config_file=CONFIG_FILE.get('reports').get('noshow'),
                     config_section='GATHER_DATA', params=[start_date, end_date])
    filename = f'report_noshow_{start_date}_{end_date}.xlsx'
    save_data_as_excel(data=df, filename=filename)
    send_mail(attachments=[filename], secret_name='EMAIL_CONFIG_REPORT_NOSHOW')

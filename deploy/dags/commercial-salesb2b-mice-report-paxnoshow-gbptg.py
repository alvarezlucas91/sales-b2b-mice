...
default_args = {
    ...
    "start_date": datetime.now(tz=local_tz) - timedelta(days=31),
    "email": [],
    "email_on_failure": "",
...
with models.DAG(
        ...
        schedule_interval=None if environment == "dev" else "0 10 1 * *",
        tags=["commercial", "salesb2b", "mice", "paxnoshow_gbptg"],
        catchup=False,
        params={"start_date": Param(
            str(date.today().replace(day=1) - timedelta(days=(date.today().replace(day=1) - timedelta(days=1)).day)),
            type="string",
            format="date",
            title="Start Date",
            ...
            "end_date": Param(
                str(date.today().replace(day=1) - timedelta(days=1)),
                type="string",
                format="date",
                title="End Date",
    ...
    run_pod = KubernetesPodOperator(
        task_id="salesb2b-mice-report-paxnoshowgbptg",
        ...
        arguments=['-m', 'reports.paxnoshow_gbptg', '--start_date', "{{ params.start_date }}", '--end_date',
                   "{{ params.end_date }}"],
        env_vars={
        ...
        container_resources=define_capacity("XS_container_resources"),

...
with models.DAG(
        ...
        schedule_interval=None if environment == "dev" else "30 17 * * 1",
        tags=["commercial", "salesb2b", "mice", "report", "credito_individual"],
        catchup=False,
        params={"booking_date": Param(
            str('2020-04-01'),
            type="string",
            format="date",
            title="Booking Date",
            description="Please select the booking date.",
        )
        }
) as dag:
    ...
    run_pod = KubernetesPodOperator(
        task_id="salesb2b-mice-report-credito-individual",
        ...
        arguments=['-m', 'reports.credito_individual', '--booking_date', "{{ params.booking_date }}"],
        env_vars={

...
# Variables que necesitamos en el main
num_iata = 'U56191919'
nom_iata = 'TURISMO_SOCIAL'
...
with models.DAG(
        ...
        schedule_interval=None if environment == "dev" else "30 9 * * *",
        tags=["commercial", "salesb2b", "mice", "report", "turismo_social"],
        catchup=False,
        params={"today_date": Param(
            str(date.today()),
            type="string",
            format="date",
            title="Start date",
    ...
    run_pod = KubernetesPodOperator(
        task_id="salesb2b-mice-report-turismo-social",
        ...
        arguments=['-m', 'reports.ttoo', '--num_iata', num_iata, '--nom_iata', nom_iata, '--today_date',
                   "{{params.today_date}}"],
        env_vars={

...
default_args = {
    ...
    "start_date": datetime.now(tz=local_tz) - timedelta(hours=2),
    "email": [],
    "email_on_failure": "",
...
with models.DAG(
        ...
        schedule_interval=None if environment == "dev" else "0 8-19/2 * * *",
        tags=["commercial", "salesb2b", "mice", "report", "monitorizacion_bundles_dos_horas", "ods-nav"],
        catchup=False
) as dag:
    ...
    run_pod = KubernetesPodOperator(
        task_id="salesb2b-mice-report-monitorizacion-bundles-dos-horas",
        ...
        arguments=f'-m reports.monitorizacion_bundles_dos_horas'.split(),
        env_vars={

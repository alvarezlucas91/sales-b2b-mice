...
with models.DAG(
        ...
        schedule_interval=None if environment == "dev" else "30 8 * * 1",
        tags=["commercial", "salesb2b", "mice", "report", "pmr_alc"],
        catchup=False,
        params={"start_date": Param(
            ...
            "end_date": Param(
                str(date.today() + relativedelta(months=1)),
                type="string",
                format="date",
                title="End Date",
    ...
    run_pod = KubernetesPodOperator(
        task_id="salesb2b-mice-report-pmr-alc",
        ...
        arguments=['-m', 'reports.pmr_alc','--start_date','{{params.start_date}}','--end_date','{{params.end_date}}'],
        env_vars={

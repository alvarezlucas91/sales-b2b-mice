...
with models.DAG(
        ...
        schedule_interval=None if environment == "dev" else "30 8 3 * *",
        tags=["commercial", "salesb2b", "mice", "report", "ssr_ttoo"],
        catchup=False,
        params={"start_date": Param(
            str(date.today() - relativedelta(months=6, day=1)),
            type="string",
            format="date",
            title="Start Date",
    ...
    run_pod = KubernetesPodOperator(
        task_id="salesb2b-mice-report-ssr-ttoo",
        ...
        arguments=['-m', 'reports.ssr_ttoo', '--start_date', '{{params.start_date}}', '--end_date',
                   '{{params.end_date}}'],
        env_vars={

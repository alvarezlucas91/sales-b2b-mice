...
default_args = {
    ...
    "start_date": datetime.now(tz=local_tz) - timedelta(days=7),#Cambio a schedule semanal
    "email": [],
    "email_on_failure": "",
...
with models.DAG(
        ...
        schedule_interval=None if environment == "dev" else "0 9 * * 3",
        tags=["commercial", "salesb2b", "mice", "report", "CRUCEROS"],
        catchup=False
) as dag:
    ...
    run_pod = KubernetesPodOperator(
        task_id="salesb2b-mice-report-cruceros",
        ...
        arguments=f'-m reports.cruceros'.split(),
        env_vars={

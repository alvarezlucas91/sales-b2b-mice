...
tableau_id = Variable.get("TABLEAU_DATASOURCE_GROUP_ALLOTMENTS")
...
with models.DAG(
        ...
        schedule_interval=None if environment == "dev" else '15 10 * * *',
        tags=[f"{area}", f"{product}", f"{project}", "allotments"],
        catchup=False,
    ...
    # https://github.com/apache/airflow/discussions/25948

    run_pod = KubernetesPodOperator(
        task_id="salesb2b-mice-group-allotments",
        ...
        arguments=['groups_allotment_main.py', '--start_date', "{{ params.start_date }}", '--end_date',
                   "{{ params.end_date }}"],
        env_vars={
        ...
        container_resources=define_capacity("L_container_resources"),
    ...
    trigger_sandbox = TriggerDagRunOperator(task_id='trigger_sync_sandbox_revenue',
                                            trigger_dag_id="commercial-salesb2b-mice-copy-data-to-revenue-sandbox")

    start >> run_pod
    run_pod >> tableau_refresh >> [finish, ko]
    run_pod >> trigger_sandbox >> [finish, ko]

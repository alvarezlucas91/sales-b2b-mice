...
with models.DAG(
        ...
        tags=[f"{area}", f"{product}", f"{project}", "report", "confirmacion","seguros"],
        catchup=False
) as dag:
    ...
    run_pod = KubernetesPodOperator(
        task_id="salesb2b-mice-report-confirmacion_seguros",
        ...
        arguments=f'-m reports.confirmacion_seguros'.split(),
        env_vars={
        ...
        labels={"app.kubernetes.io/component": "{{ task.task_id }}",
                "tags.datadoghq.com/service": f"{area}-{product}-{project}", "tags.datadoghq.com/env": environment
                },

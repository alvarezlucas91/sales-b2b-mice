...
with models.DAG(
        ...
        tags=["commercial", "salesb2b", "mice", "report", "noshow"],
        catchup=False,
        params={"start_date": Param(
            str(date(year=2024, month=1, day=1)),
            type="string",
            format="date",
            title="Start Date",
    ...
    run_pod = KubernetesPodOperator(
        task_id="salesb2b-mice-report-noshow",
        ...
        arguments=['-m', 'reports.noshow', '--start_date', '{{params.start_date}}', '--end_date',
                   '{{params.end_date}}'],
        env_vars={

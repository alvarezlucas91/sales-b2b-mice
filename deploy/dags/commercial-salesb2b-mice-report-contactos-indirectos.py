...
with models.DAG(
        ...
        schedule_interval=None if environment == "dev" else "15 9 3 * *",
        tags=["commercial", "salesb2b", "mice", "report", "contactos_indirectos"],
        catchup=False,
        params={"start_date": Param(str(date.today().replace(day=1) - relativedelta(months=1)),
            type="string",
            format="date",
            title="Start Date",
    ...
    run_pod = KubernetesPodOperator(
        task_id="salesb2b-mice-report-contactos-indirectos",
        ...
        arguments=['-m', 'reports.contactos_indirectos', '--start_date', '{{params.start_date}}', '--end_date',
                   '{{params.end_date}}'],
        env_vars={

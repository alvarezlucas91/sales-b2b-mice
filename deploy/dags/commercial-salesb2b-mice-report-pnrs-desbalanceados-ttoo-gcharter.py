...
with models.DAG(
        ...
        schedule_interval=None if environment == "dev" else "0 9 * * *",
        tags=["commercial", "salesb2b", "mice", "pnrs_desbalanceados_ttoo_gcharter"],
        catchup=False,
    ...
    run_pod = KubernetesPodOperator(
        task_id="salesb2b-mice-report-pnrsdesbalanceadosttoogcharter",
        ...
        arguments=['-m', 'reports.pnrs_desbalanceados_ttoo_gcharter', '--start_date', "{{ params.start_date }}", '--end_date',
                   "{{ params.end_date }}"],
        env_vars={
